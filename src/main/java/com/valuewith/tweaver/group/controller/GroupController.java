...
public class GroupController {
  ...
  public ResponseEntity<?> writeGroup(GroupDto groupDto) {
    // 1.그룹 등록
    Group group = groupService.writeGroup(groupDto);
    // 2.여행 등록
    placeService.writePlace(group, groupDto.getPlaces());
    // 3.채팅 등록
    ChatRoom chatRoom = chatRoomService.setChatRoom(group);
    ...
    User user = User.builder()
        ...
        .isSocial(true)
        .build();
    memberService.setMember(group, user, chatRoom);
